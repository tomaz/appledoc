...
#pragma mark - Multiple method definitions

- (void)testParseStreamForParserStoreShouldDetectMultipleMethodDefinitions {
	[self runWithState:^(ObjectiveCMethodState *state) {
		[self runWithFile:@"MethodStateMultipleDefinitions.h" block:^(id parser, id tokens) {
			// setup
			id store = [OCMockObject mockForClass:[Store class]];
			[[store expect] setCurrentSourceInfo:OCMOCK_ANY];
			[[store expect] beginMethodDefinitionWithType:GBStoreTypes.instanceMethod];
