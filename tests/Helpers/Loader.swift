...
class Loader {
	...
	class func path(_ filename: String, example: String) -> PathInfo {
		...
		if let match = headerRegex.firstMatch(in: contents, options: RegularExpression.MatchingOptions(rawValue: 0), range: searchRange) {
	...
	private class func pathString(_ filename: String) -> String {
		return Bundle(for: Loader.self).pathForResource(filename.stringByDeletingPathExtension, ofType: filename.pathExtension)!
	}
	
	private static var headerRegex = {
		return try! RegularExpression(pattern: "^###.+$\\n?", options: RegularExpression.Options.anchorsMatchLines)
	}()
