...
class Settings: GBSettings {
	...
	func injectGlobalSettings() {
		...
		if ((Settings.globalDefaults?.loadSettingsFromFile(userRootPath)) != nil) {
	...
	func injectProjectSettings() {
		...
		for path in self.arguments {
			...
			if ((Settings.projectDefaults?.loadSettingsFromFile(settingsPath)) != nil) {
...
extension Settings {
	...
	private func loadSettingsFromFile(_ path: String) -> Bool {
	...
	private class func settingsFileNameForPath(_ path: String) -> String {
