...
uint32_t ZCALLBACK fread_buf_func(voidpf opaque, voidpf stream, void *buf, uint32_t size)
{
    ...
    while (bytes_left_to_read > 0)
    {
        ...
        if ((streamio->readbuf_len - streamio->readbuf_pos) > 0)
        {
            ...
            bytes_to_copy = bytes_left_to_read < (uint32_t)(streamio->readbuf_len - streamio->readbuf_pos) ? bytes_left_to_read : (uint32_t)(streamio->readbuf_len - streamio->readbuf_pos);            
//            min(bytes_left_to_read, (uint32_t)(streamio->readbuf_len - streamio->readbuf_pos));
...
uint32_t ZCALLBACK fwrite_buf_func(voidpf opaque, voidpf stream, const void *buf, uint32_t size)
{
    ...
    while (bytes_left_to_write > 0)
    {
        bytes_to_copy = bytes_left_to_write < (uint32_t)(IOBUF_BUFFERSIZE -
        (streamio->writebuf_len < streamio->writebuf_pos ? streamio->writebuf_len : streamio->writebuf_pos))
        ? bytes_left_to_write : (uint32_t)(IOBUF_BUFFERSIZE -
                                           (streamio->writebuf_len < streamio->writebuf_pos ? streamio->writebuf_len : streamio->writebuf_pos));
