...
- (NSString *)formatLogMessage:(DDLogMessage *)m {
    // TODO: originalFilename isn't available.
    if (m->file) {
        NSString *level = nil;
        switch (m->logFlag) {
        ...
        return [NSString stringWithFormat:@"%@:%d: %@: %@", [NSString stringWithCharacters:(unichar *)m->file length:strnlen(m->file, 1024)], m->lineNumber, level, GBLogMessage(m)];
    }
