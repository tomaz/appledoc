...
- (BOOL)moveSourceFilesToDocuments:(NSError **)error {
	...
	// Move all files generated by previous generator to documents subfolder of docset structure. Note that we need to remove document dir before or we'll get an error.
	if (![self.fileManager removeItemAtPath:[documentsPath stringByStandardizingPath] error:error]) {
		GBLogWarn(@"Failed removing '%@'!", documentsPath);
		return NO;
	}
	if (![self.fileManager moveItemAtPath:sourceFilesPath toPath:[documentsPath stringByStandardizingPath] error:error]) {
